'use client';

import { Card } from '@/components/ui/card';
import { DateSelect } from '@/components/common/DateSelect';
import EmptyTableData from '@/components/common/EmptyTableData';
import SearchInput from '@/components/common/forms/SearchInput';
import AddDepositCategory from './AddDepositCategory';
import DepositCategoryTable from './DepositCategoryTable';
import { FILTER_OPTIONS, SORT_OPTIONS } from '@/config/data';
import PaginateAction from '@/components/common/PaginateAction';
import { IDepositCategoryData } from '@/types/deposit-category-interface';
import SortAndFilterComponent from '@/components/common/SortAndFilterComponent';

const DepositCategoryPageWrapper = ({
  depositCategories,
}: {
  depositCategories: IDepositCategoryData;
}) => {
  return (
    <Card className="p-space16">
      <div className="flex justify-between items-center border-b border-gray-200 pb-space16">
        <div>
          {' '}
          <h1 className="text-md font-semibold">Deposit Category</h1>
        </div>
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-2">
            <SearchInput wrapperClasses="h-[40px]" />
            {/* <div className="relative flex">
              <SortAndFilterComponent
                label="Sort"
                triggerClassName="rounded-r-none"
                fields={SORT_OPTIONS}
                containerClassName="w-54 border-r-none"
              />
              <SortAndFilterComponent
                fields={FILTER_OPTIONS}
                triggerClassName="rounded-l-none"
                label="Filter"
              />
            </div>
            <DateSelect /> */}
            <AddDepositCategory />
          </div>
        </div>
      </div>

      {depositCategories?.data?.length > 0 ? (
        <DepositCategoryTable
          depositCategories={depositCategories?.data}
          activePage={depositCategories?.current_page ?? 0}
          perPage={depositCategories?.per_page ?? 0}
        />
      ) : (
        <EmptyTableData
          title="There is no deposit category created here yet!"
          description="Add deposit category to your shop and adjust them as you wish. You will be able to add, update and delete whenever you want, whenever you wish"
          action={<AddDepositCategory />}
        />
      )}
      <PaginateAction
        total={depositCategories?.total ?? 0}
        perPage={depositCategories?.per_page ?? 0}
        activePage={depositCategories?.current_page ?? 0}
      />
    </Card>
  );
};

export default DepositCategoryPageWrapper;
