'use client';

import { Card } from '@/components/ui/card';
import { DateSelect } from '@/components/common/DateSelect';
import EmptyTableData from '@/components/common/EmptyTableData';
import SearchInput from '@/components/common/forms/SearchInput';
import SortAndFilterComponent from '@/components/common/SortAndFilterComponent';
import { ILeaveTypesData } from '@/types/leave-type-interface';
import LeaveTypeTable from './LeaveTypeTable';
import AddLeaveType from './AddLeaveType';
import { FILTER_OPTIONS, SORT_OPTIONS } from '@/config/data';

const LeaveTypePageWrapper = ({
  leaveTypes,
}: {
  leaveTypes: ILeaveTypesData;
}) => {
  return (
    <Card className="p-space16">
      <div className="flex justify-between items-center border-b border-gray-200 pb-space16">
        <div>
          {' '}
          <h1 className="text-md font-semibold">Leave Types</h1>
        </div>
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex gap-2">
            <SearchInput wrapperClasses="h-[40px]" />
            <div className="relative flex">
              <SortAndFilterComponent
                label="Sort"
                triggerClassName="rounded-r-none"
                fields={SORT_OPTIONS}
                containerClassName="w-54 border-r-none"
              />
              <SortAndFilterComponent
                fields={FILTER_OPTIONS}
                triggerClassName="rounded-l-none"
                label="Filter"
              />
            </div>
            <DateSelect />
            <AddLeaveType />
          </div>
        </div>
      </div>

      {leaveTypes?.data?.length > 0 ? (
        <LeaveTypeTable leaveTypes={leaveTypes}
          activePage={leaveTypes?.current_page ?? 0}
          perPage={leaveTypes?.per_page ?? 0}
        />
      ) : (
        <EmptyTableData
          title="There is no leave type created here yet!"
          description="Create leave type to your shop and adjust them as you wish. You will be able to add, update and delete whenever you want, whenever you wish"
          action={<AddLeaveType />}
        />
      )}
    </Card>
  );
};

export default LeaveTypePageWrapper;
