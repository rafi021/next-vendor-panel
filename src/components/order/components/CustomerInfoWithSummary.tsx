...
export const CustomerInfoView = ({
  ...
  console.log('customer last => ', customer_info);
      ...
      <Card className="border-none shadow-none px-space6">
        ...
        <article className="text-sm flex flex-col gap-space6 py-space24">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-space8">
              <p>{customer_info?.total_orders}</p>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <p className="text-gray-500">Delivered</p>
            <div className="flex items-center gap-space8">
              <p>{customer_info?.delivered_orders}</p>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <p className="text-gray-500">Cancelled</p>
            <div className="flex items-center gap-space8">
              <p>{customer_info?.cancelled_orders}</p>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <p className="text-gray-500">Return</p>
            <div className="flex items-center gap-space8">
              <p>{customer_info?.returned_orders}</p>
            </div>
          </div>
...
export const CustomerInfoWithSummary = ({
  customer,
  customer_info,
}: {
  ...
  const fetchStatus = async () => {
    ...
    startTransition(async () => {
      ...
      const statusData = customer.phone
        ? await getSuccessRateByPhone(customer.phone)
        : null;
      if (statusData) {
